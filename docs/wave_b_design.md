...
| 12 | IPv6 egress timeout на Vultr-нодах: ipapi.co резолвится в IPv4, IPv6-only ноды не достучатся → периодический null geo_country и ipv6.ok=false в /health | infrastructure (Vultr) | **infrastructure issue, not in code**: watchdog (Wave B-7a) auto-recover'ит unstable ноды; long-term — multi-provider или alternative geo-lookup over IPv6 |
...
| 15 | /opt/netrun на production-нодах НЕ git-repo (install_node.sh копирует файлы без .git) — нельзя `git pull` для обновления | `node_runtime/install_node.sh` | Wave B-9 (deferred): написать `update_node.sh` который делает tmp-clone + rsync diff в /opt/netrun |
| 16 | geo_code null от ipapi.co на IPv6-only нодах (IPv4 outbound заблокирован, ipapi.co не резолвится) | `node_runtime` /describe handler | Wave B-9: alternative geo lookup over IPv6, либо enroll-node `--geo XX` остаётся обязательным workaround'ом |
| 17 | Orchestrator port 8090 publicly accessible (`ORCHESTRATOR_HOST=0.0.0.0`, no firewall). Bot scanners attempt RCE via Log4Shell-style URLs (404'd, but visible in logs) | `install_orchestrator.sh` / `.env` | **FIXED in B-7b.5**: install_orchestrator.sh default `ORCHESTRATOR_HOST=127.0.0.1` (existing .env preserved on re-run); opt-in `scripts/install_nginx.sh` installs nginx reverse proxy on port 8091 with explicit `server_name` (no `default_server` — coexists with other projects) and metrics ACL via `METRICS_ALLOW_NETWORKS` env. TLS via certbot is manual. |
